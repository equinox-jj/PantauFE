...
part 'feed_reports_notifier.g.dart';
...
/// A report plus its distance from the device, precomputed so the card renders
/// without doing geometry in `build`.
...
@riverpod
class FeedReports extends _$FeedReports {
  /// Origin of the most recent [load], replayed by [refresh].
  double? _latitude;
  double? _longitude;
  ...
  /// Starts loading rather than empty: the page locates on mount, so an empty
  /// list before the first response would render as "nothing nearby".
  @override
  AsyncValue<List<FeedItem>> build() => const AsyncLoading();
  ...
  Future<void> load({
    ...
    // Only a load with nothing on screen announces itself. A refetch keeps the
    // previous cards — the pull-to-refresh spinner is the progress signal, and
    // swapping the list for skeletons would throw the scroll position away.
